...
handle_direct_connection() {
    ...
    # --- Parse Arguments ---
    # A simple parser that expects user@host and optional -p port
    while (( "$#" )); do
        case "$1" in
            -p)
                if [[ -n "$2" && "$2" != -* ]]; then
    ...
    echo "New host detected. Testing connection to ${user_host} on port ${port}..."

    # Stage 1: Try with public keys only
    if ssh -o PasswordAuthentication=no -o ConnectTimeout=5 "${user}@${hostname}" -p "${port}" "exit"; then
        echo "Connection successful with SSH key."
        key_installed=1
    # Stage 2: If keys fail, try with password
    elif ssh -o ConnectTimeout=5 "${user}@${hostname}" -p "${port}" "exit"; then
        echo "Connection successful with password."
        key_installed=0
    else
        echo "Connection failed. Please check the host details and your credentials."
        return 1
    fi

    # --- Save New Host ---
    read -p "Connection successful. Enter a friendly name to save this host: " friendly_name
