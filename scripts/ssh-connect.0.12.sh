...
handle_direct_connection() {
    ...
    # Redirect all output to /dev/null to ignore banners.
    if ssh -o BatchMode=yes -o ConnectTimeout=5 "${user}@${hostname}" -p "${port}" "exit" &> /dev/null; then
        echo "Connection successful with SSH key."
        key_installed=1
    # Stage 2: If keys fail, try interactively (allows password).
    # Still redirect output to ignore banners.
    elif ssh -o ConnectTimeout=5 "${user}@${hostname}" -p "${port}" "exit" &> /dev/null; then
