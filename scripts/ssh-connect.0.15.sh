...
connect_to_host() {
    ...
    if ssh -o ControlPath="${socket_file}" -O check "${user}@${hostname}" -p "${port}" &>/dev/null; then
        ...
        # This command runs in the foreground and will prompt for a password if needed.
        # It creates the master connection and then immediately exits.
        if ! ssh -M -o ControlPersist=10s -o ControlPath="${socket_file}" "${user}@${hostname}" -p "${port}" "exit"; then
            ...
            # Clean up the failed socket attempt
            rm -f "${socket_file}"
            return 1
        fi
    fi
    ...
    # At this point, we are guaranteed to have a live, authenticated socket.
