...
connect_to_host() {
    ...
    # Path A: Reuse an existing connection if the socket is live.
    set -x
    if ssh -o ControlPath="${socket_file}" -O check "${user}@${hostname}" -p "${port}" &>/dev/null; then
        ...
        # Path B: Create a new connection.
        echo "Establishing connection to ${friendly_name}..."
        
        # Attempt 1: Standard interactive connection.
        # This will use keys first, then fallback to password if the server allows it.
        if ! ssh -M -o ControlPersist=10s -o ControlPath="${socket_file}" "${user}@${hostname}" -p "${port}" "exit"; then
            
            # Attempt 2: Password-only fallback.
            # This is for servers that disconnect after a failed key attempt.
            echo "Initial connection failed. Retrying with password-only authentication..."
            if ! ssh -M -o ControlPersist=10s -o ControlPath="${socket_file}" -o PreferredAuthentications=password "${user}@${hostname}" -p "${port}" "exit"; then
