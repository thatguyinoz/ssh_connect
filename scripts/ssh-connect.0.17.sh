...
#
# Version 0.15:
#   - Reworked the core connection logic in 'connect_to_host' to reliably
#     handle password-based logins, especially on hosts with banners.
#   - The script now uses an interactive foreground command ('ssh -M') to
#     establish a new master connection, ensuring password prompts are
...
set -x
    ...
    if [[ "${is_edgerouter}" == "true" ]]; then
        # --- EdgeOS Specific Logic (User-Guided) ---
        echo "EdgeOS device detected. Using specialized installation method."
        local remote_tmp_key="/tmp/ssh_connect_key.pub"

        # Step 1: SCP the key to the device
        echo "Uploading public key..."
        if ! scp -P $port -o ControlPath="${socket_file}" "${selected_key}" "${user}@${hostname}:${remote_tmp_key}"; then
            echo "Error: Failed to copy key to the EdgeRouter."
            return 1
        fi
set +x
        # Step 2: Display instructions and open interactive session
        cat << EOF

-- EdgeOS Device Detected: Manual Configuration Required --

The key has been uploaded. Please run the following commands
in the interactive session that will now open:

1. configure
2. loadkey ${user} ${remote_tmp_key}
3. commit
4. save
5. exit
...
EOF
        ...
        echo "Cleaning up temporary key from device..."
        ssh -o ControlPath="${socket_file}" "${user}@${hostname}" -p "${port}" "rm ${remote_tmp_key}"
        
        # Step 4: Confirm success
        read -p "Was the key installed successfully? (y/n): " confirm
        if [[ "${confirm}" == "y" || "${confirm}" == "Y" ]]; then
            echo "Key status updated."
            update_key_status "${host_details}"
        else
            echo "Key installation not marked as complete."
        fi
