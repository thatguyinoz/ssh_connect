...
DESCRIPTION:
    ...
    Additional [ssh_options] like -L or -R for port forwarding can be added
    after the host name.
...
select_host() {
    ...
    shift # Shift away the array name
    local forwarding_flags=("$@") # The rest are the forwarding flags
...
handle_direct_connection() {
    ...
    local port=22 # Default port, direct connections don't support custom ports yet
...
main() {
    ...
    # The first argument is always the friendly name or a direct connection string.
    local friendly_name="$1"
    shift # Remove the host name from the argument list.
    # All remaining arguments are treated as flags for SSH.
    local forwarding_flags=("$@")
    ...
    if [[ -n "${host_line}" ]]; then
        ...
        # If it doesn't exist, treat it as a direct connection string (e.g., user@host).
        handle_direct_connection "${friendly_name}" "${forwarding_flags[@]}"
    fi
}
