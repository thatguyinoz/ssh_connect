...
# --- Changelog ---
# Version 0.33:
#   - Standardized CSV column documentation naming schema across the usage help,
#     configuration templates, and README.md.
#
# Version 0.32:
#   - Introduced new CLI debug mode (`-d` or `--debug`) to output the full SSH
#     commands used to establish connections.
#   - Automatically appends standard SSH handshake verbosity (`-v`) in debug
#     mode and bypasses stdout/stderr redirections to allow live diagnostic logs.
#
# Version 0.31:
#   - Upgraded host configuration schema to 9 columns, introducing custom
#     `SSHOptions` support for persistent, literal CLI flags per host.
#   - Completely overhauled argument parsing to support arbitrary SSH options
#     in any sequential order, both with or without a specified host.
#   - Added custom visual indication (🔗) in the menu for hosts using stored
#     SSH options.
#   - Enabled automatic detection and prompting to save active CLI forwarding/SSH
#     options when registering a new direct host connection.
#
# Version 0.30:
#   - Added paging support to the interactive host selection menu to
#     prevent screen overflow when listing a large number of hosts.
#     Supports customizable page size, wrapping, and navigation controls.
#
# Version 0.29:
...
USAGE:
    ${SCRIPT_NAME} [options]
    ${SCRIPT_NAME} <host_name> [ssh_options]
...
DESCRIPTION:
    ...
    The script supports standard SSH options (such as -L, -R, -c, -m, -o, -i)
    in any sequential order. Specify a connection port using -p.
    ...
    Hosts with saved SSH options/forwarding rules are marked with 🔗.

OPTIONS:
    -h, --help    Show this help message.
    -v, --version Show the script version.
    -d, --debug   Enable debug mode to show full SSH commands and verbose logs.
...
CONFIGURATION:
    Host File:
        ...
        comma-separated (CSV) format (9 columns):
        ...
        - Port:                The SSH port.
        - LastConnTimestamp:   Unix timestamp of the last connection (managed by script).
        ...
        - SSHOptions:          A literal string of standard SSH flags/options to apply
                               persistently to connections for this host.
...
# ------------------------------------------------------------------------------
# Function: load_hosts()
# Description: Loads hosts from the config file into an array.
...
# ------------------------------------------------------------------------------
# Function: display_hosts_page()
# Description: Displays a single page of hosts from the loaded array.
# ------------------------------------------------------------------------------
display_hosts_page() {
    local -n _dh_hosts_ref=$1
    local page=$2
    local page_size=10
    local total_hosts=${#_dh_hosts_ref[@]}
    local total_pages=$(( (total_hosts + page_size - 1) / page_size ))

    # 1. Get current terminal width (default to 80 if tput is not available)
    local term_cols
    term_cols=$(tput cols 2>/dev/null || echo 80)

    # 2. Find the maximum display name length across ALL hosts to ensure stable column alignment
    local max_display_len=0
    for host_line in "${_dh_hosts_ref[@]}"; do
        IFS=',' read -r friendly_name _ _ _ _ key_installed jump_host_name _ ssh_options <<< "${host_line}"
        ...
        if (( display_len > max_display_len )); then
            max_display_len=${display_len}
        fi
    done

    # 3. Calculate column width and number of columns that can fit
    # Add 8 characters buffer (for '%2d. ' numbering prefix and padding spaces)
    local col_width=$(( max_display_len + 8 ))
    if (( col_width < 20 )); then
        col_width=20 # Set a sensible minimum column width
    fi
    ...
    echo "Available hosts (Page ${page}/${total_pages}):"

    # 4. Determine page boundary indices
    local start_idx=$(( (page - 1) * page_size ))
    local end_idx=$(( start_idx + page_size ))
    if (( end_idx > total_hosts )); then
        end_idx=${total_hosts}
    fi

    # 5. Print the hosts in a horizontal grid
    local col_count=0
    for (( i=start_idx; i<end_idx; i++ )); do
        local host_line="${_dh_hosts_ref[i]}"
        IFS=',' read -r friendly_name _ _ _ _ key_installed jump_host_name _ ssh_options <<< "${host_line}"
        ...
        # Build the cell text
        local prefix_str=$(printf "%2d. " "$((i+1))")
        local cell_str="${prefix_str}${display_name}"
        
        # Calculate manual space padding to keep alignment consistent across terminals
        local cell_len=${#cell_str}
        local pad_len=$(( col_width - cell_len ))
        local spaces=""
        if (( pad_len > 0 )); then
            spaces=$(printf '%*s' "${pad_len}" "")
        fi
        
        printf "%s%s" "${cell_str}" "${spaces}"
        
        col_count=$(( col_count + 1 ))
        if (( col_count == num_cols )); then
            echo ""
            col_count=0
        fi
    done

    # If the last row was not filled to num_cols, print the trailing newline
    if (( col_count != 0 )); then
        echo ""
    fi
}
...
#              and initiates the connection. Supports paging through 'n' and 'p'.
# ------------------------------------------------------------------------------
select_host() {
    ...
    local page_size=10
    local total_hosts=${#hosts_ref[@]}
    local total_pages=$(( (total_hosts + page_size - 1) / page_size ))
    local current_page=1

    # Display the first page of hosts
    display_hosts_page hosts_ref "${current_page}"

    while true; do
        local prompt_str="Enter host number"
        if (( total_pages > 1 )); then
            prompt_str+=", 'n'/Enter (next), 'p' (prev)"
        fi
        prompt_str+=", 'h' for help, or 'q' to quit: "

        read -p "${prompt_str}" selection
        case "${selection}" in
            ""|n|next)
                if (( total_pages > 1 )); then
                    if (( current_page < total_pages )); then
                        ...
                        current_page=1 # Wrap around
                    fi
                    echo ""
                    display_hosts_page hosts_ref "${current_page}"
                else
                    # If they typed "n" or "next" specifically, let them know there's only one page.
                    # If they just hit Enter on a single-page list, fail silently to keep it clean.
                    if [[ -n "${selection}" ]]; then
                        echo "Only one page of hosts is available."
                    fi
                fi
                ;;
            p|prev)
                if (( total_pages > 1 )); then
                    if (( current_page > 1 )); then
                        ...
                        current_page=${total_pages} # Wrap around
                    fi
                    echo ""
                    display_hosts_page hosts_ref "${current_page}"
                else
            ...
            h|help)
                usage
                # After showing help, re-display the current page and re-prompt.
                echo -e "\n---"
                display_hosts_page hosts_ref "${current_page}"
                ;;
            q|quit)
                ...
                ;;
            [0-9]*)
                if (( selection >= 1 && selection <= total_hosts )); then
                    ...
                    echo "Invalid selection. Please enter a number between 1 and ${total_hosts}."
                fi
                ;;
            *)
                ...
                error_msg+=", 'h' for help, or 'q' to quit."
                echo "${error_msg}"
                ;;
        esac
    done
...
connect_to_host() {
    ...
    local friendly_name user hostname port timestamp key_installed jump_host_name is_jumphost ssh_options
    IFS=',' read -r friendly_name user hostname port timestamp key_installed jump_host_name is_jumphost ssh_options <<< "${host_details}"
    
    # Parse persistent custom SSH options from the host entry
    local persistent_flags=()
    if [[ -n "${ssh_options}" ]]; then
        read -ra persistent_flags <<< "${ssh_options}"
    fi

    # Combine persistent options with ad-hoc CLI flags
    local final_flags=("${persistent_flags[@]}" "${forwarding_flags[@]}")
    ...
    if ssh ${jump_flag} -o ControlPath="${socket_file}" -O check "${user}@${hostname}" -p "${port}" &>/dev/null; then
        ...
        if [[ "${DEBUG_MODE}" == "true" ]]; then
            echo -e "\n[DEBUG] Establishing persistent connection using:"
            echo "ssh -v ${jump_flag} -M -o ControlPersist=10s -o ControlPath=\"${socket_file}\" \"${user}@${hostname}\" -p \"${port}\" \"exit\""
            
            # Attempt 1: Standard key-based authentication with live handshake verbose logs
            if ! ssh -v ${jump_flag} -M -o ControlPersist=10s -o ControlPath="${socket_file}" "${user}@${hostname}" -p "${port}" "exit"; then
                echo "Initial connection failed. Retrying with password-only authentication..."
                echo -e "\n[DEBUG] Establishing persistent connection (password fallback) using:"
                echo "ssh -v ${jump_flag} -M -o ControlPersist=10s -o ControlPath=\"${socket_file}\" -o PreferredAuthentications=password \"${user}@${hostname}\" -p \"${port}\" \"exit\""
                
                # Attempt 2: Password-only fallback
                if ! ssh -v ${jump_flag} -M -o ControlPersist=10s -o ControlPath="${socket_file}" -o PreferredAuthentications=password "${user}@${hostname}" -p "${port}" "exit"; then
    ...
    if [[ "${DEBUG_MODE}" == "true" ]]; then
        echo -e "\n[DEBUG] Spawning interactive session using:"
        echo "ssh -v \"${final_flags[@]}\" ${jump_flag} -o ControlPath=\"${socket_file}\" \"${user}@${hostname}\" -p \"${port}\""
        exec ssh -v "${final_flags[@]}" ${jump_flag} -o ControlPath="${socket_file}" "${user}@${hostname}" -p "${port}"
    elif [[ -n "${TERMINAL_CMD}" && -x "$(command -v ${TERMINAL_CMD})" ]]; then
        echo "Opening new terminal with port forwarding..."
        ${TERMINAL_CMD} ssh "${final_flags[@]}" ${jump_flag} -o ControlPath="${socket_file}" "${user}@${hostname}" -p "${port}" &
    else
        echo "Spawning session in current terminal with port forwarding..."
        exec ssh "${final_flags[@]}" ${jump_flag} -o ControlPath="${socket_file}" "${user}@${hostname}" -p "${port}"
    fi
}
...
handle_direct_connection() {
    ...
    # --- Check for Custom SSH Options / Forwarding Flags ---
    local saved_ssh_options=""
    if (( ${#forwarding_flags[@]} > 0 )); then
        local display_flags="${forwarding_flags[*]}"
        read -p "Do you want to save these SSH options '${display_flags}' as persistent rules for this host? [y/N]: " save_flags_choice
        if [[ "$save_flags_choice" == "y" || "$save_flags_choice" == "Y" ]]; then
            saved_ssh_options="${display_flags}"
            # Since we are saving them permanently in the host config, we clear the ad-hoc forwarding_flags
            # to prevent them from being duplicated when connect_to_host is called.
            forwarding_flags=()
        fi
    fi

    # --- Save New Host ---
    local new_host_line="${friendly_name},${user},${hostname},${port},0,${key_installed},${jump_host_name},${is_jumphost},${saved_ssh_options}"
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            -p*)
                # Handles both -p 2222 and -p2222 regardless of sequential order
                val="${1#-p}"
                if [[ -z "${val}" ]]; then
                    if [[ -z "$2" || "$2" =~ ^- ]]; then
                ...
                ;;
            *)
                # Sequentially accumulate any other flag/argument as-is
                forwarding_flags+=("$1")
                shift
                ;;
        esac
